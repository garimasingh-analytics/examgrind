...
export default function VaultStudio({ subjects, initialItems }: { subjects: Subject[]; initialItems: VaultItem[] }) {
  ...
  return <>
    <section className="vault-cover rounded-3xl border border-cocoa-900/[.12] bg-cocoa-900 p-5 text-cream-50 shadow-warm-lg sm:p-7">
      ...
      <div className="mt-6 rounded-2xl border border-cream-50/15 bg-cream-50/10 p-3 sm:flex sm:items-center sm:gap-3">
...
function FlashcardSession({ set, onClose }: { set: VaultItem; onClose: () => void }) {
  ...
  return <div className="mt-5 rounded-2xl border border-ember-600/15 bg-sun-300/10 p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-ember-700">Study mode · {cardIndex + 1} of {cards.length}</p><h3 className="mt-1 font-serif text-xl font-bold text-cocoa-900">{set.topic}</h3></div><button type="button" onClick={onClose} className="text-xs font-bold text-cocoa-600 hover:text-cocoa-900">Close</button></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-cocoa-900/10"><div className="h-full rounded-full bg-ember-600 transition-all" style={{ width: `${((cardIndex + 1) / cards.length) * 100}%` }} /></div><button type="button" aria-pressed={flipped} aria-label={flipped ? "Hide answer" : "Reveal answer"} onClick={() => setFlipped((value) => !value)} className="mt-5 flex min-h-56 w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-ember-600/25 bg-white px-5 py-7 text-center shadow-warm transition hover:-translate-y-0.5 hover:border-ember-600/50 focus:outline-none focus:ring-2 focus:ring-ember-600"><span className="text-[11px] font-bold uppercase tracking-[.18em] text-ember-700">{flipped ? "Answer" : "Question"}</span><span className="mt-4 max-w-xl font-serif text-xl font-bold leading-relaxed text-cocoa-900 sm:text-2xl">{flipped ? card.back : card.front}</span><span className="mt-5 text-xs font-semibold text-cocoa-500">{flipped ? `Hint: ${card.hint}` : "Tap the card to reveal the answer"}</span></button><div className="mt-4 flex items-center justify-between gap-3"><button type="button" disabled={cardIndex === 0} onClick={() => move(-1)} className="rounded-xl border border-cocoa-900/[.12] bg-white px-3 py-2 text-sm font-bold text-cocoa-900 disabled:cursor-not-allowed disabled:opacity-40">← Previous</button><button type="button" onClick={() => setFlipped((value) => !value)} className="rounded-xl bg-sun-400 px-3 py-2 text-sm font-extrabold text-cocoa-900">{flipped ? "Show question" : "Flip card"}</button><button type="button" onClick={() => move(1)} className="rounded-xl bg-cocoa-900 px-3 py-2 text-sm font-extrabold text-cream-50">{cardIndex === cards.length - 1 ? "Finish" : "Next →"}</button></div></div>;
}
