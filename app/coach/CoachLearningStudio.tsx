...
export default function CoachLearningStudio({ topics, priorityTopicIds, recentLessons }: { topics: LearnTopic[]; priorityTopicIds: string[]; recentLessons: RecentLesson[] }) {
  ...
  return <section id="learn-with-coach" className="mx-auto mt-5 max-w-4xl px-5">
    <div className="overflow-hidden rounded-3xl border border-cocoa-900 bg-[#1D1815] p-5 text-cream-50 shadow-warm-lg sm:p-7">
      <p className="text-xs font-bold uppercase tracking-[.18em] text-sun-300">Coach lesson studio</p>
      <div className="mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><h2 className="font-serif text-3xl font-bold leading-tight text-white">Learn a topic. Then practise it.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-cream-50">Choose a syllabus topic for a full lesson, or ask Coach about one specific concept. Each lesson ends with practice on what you just learned.</p></div>
      ...
      <div className="mt-6 rounded-2xl border border-sun-300/45 bg-cream-50/[.11] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,.08)]">
        <label htmlFor="coach-direct-topic" className="text-[11px] font-bold uppercase tracking-[.14em] text-sun-200">Ask Coach about any exact concept</label>
        ...
        <p className="mt-2 text-xs text-cream-100/70">Type exactly what you want to understand. Coach can explain a full topic or one small concept.</p>
      </div>

      {topics.length > 0 && <details className="mt-4 rounded-2xl border border-cream-50/25 bg-cream-50/[.08] p-3"><summary className="cursor-pointer text-sm font-extrabold text-cream-50">Or teach a full syllabus topic</summary>
        {suggestedTopics.length > 0 && <div className="mt-4"><p className="text-[11px] font-bold uppercase tracking-[.15em] text-cream-100/65">Suggested from your evidence</p><div className="mt-2 flex flex-wrap gap-2">{suggestedTopics.map((topic) => <button key={topic.id} type="button" disabled={pending} onClick={() => { setSelectedTopicId(topic.id); buildLesson({ topicId: topic.id }); }} className="rounded-full border border-white/20 bg-white/10 px-3 py-2 text-left text-xs font-bold text-cream-50 transition hover:bg-white/20 disabled:opacity-60">{topic.name} <span className="font-medium text-cream-100/65">· {topic.subjectName}</span></button>)}</div></div>}
      ...
      {recentLessons.length > 0 && <div className="mt-4 rounded-2xl border border-cream-50/25 bg-cream-50/[.08] p-3"><p className="text-[11px] font-bold uppercase tracking-[.15em] text-cream-100/75">Continue a recent Coach lesson</p><div className="mt-2 flex flex-wrap gap-2">{recentLessons.map((item) => <button key={item.id} type="button" disabled={pending} onClick={() => { setDirectTopic(item.requested_topic); buildLesson(item.topic_id ? { topicId: item.topic_id } : { directTopic: item.requested_topic }); }} className="rounded-full border border-cream-50/25 bg-cream-50/[.12] px-3 py-2 text-left text-xs font-bold text-cream-50 transition hover:bg-cream-50/20 disabled:opacity-60">{item.requested_topic} <span className="font-medium text-cream-100/75">· reopen</span></button>)}</div></div>}
      {error && <p role="alert" className="mt-3 rounded-xl bg-coral-500/20 px-3 py-2 text-sm font-semibold text-cream-50">{error}</p>}
      <p className="mt-3 text-xs text-cream-100/65">Each lesson explains the idea, checks your understanding and gives you practice.</p>
    </div>
