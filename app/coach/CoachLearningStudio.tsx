...
function LessonCanvas({ lesson, topic, selectedAnswer, onChooseAnswer }: { lesson: CoachLesson; topic: LearnTopic; selectedAnswer: number | null; onChooseAnswer: (index: number) => void }) {
  ...
  return <article className="mt-5 overflow-hidden rounded-3xl border border-cocoa-900/[.08] bg-cream-50 shadow-warm-lg">
    ...
    <div className="p-5 sm:p-6"><CoachVisualWalkthrough lesson={lesson} topic={topic} /><p className="mt-6 text-xs font-bold uppercase tracking-[.16em] text-cocoa-500">Learn the pieces, in order</p><ol className="mt-4 grid gap-3 md:grid-cols-2">{lesson.steps.map((step, index) => <li key={`${step.title}-${index}`} className="relative overflow-hidden rounded-2xl border border-violet-700/15 bg-violet-700/[.045] p-4"><div className="flex items-center gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-violet-700 text-xs font-bold text-white">{index + 1}</span><p className="text-[11px] font-bold uppercase tracking-[.13em] text-violet-700">{step.visualLabel}</p></div><h4 className="mt-4 font-serif text-xl font-bold text-cocoa-900">{step.title}</h4><p className="mt-2 text-sm leading-6 text-cocoa-700">{step.explanation}</p></li>)}</ol>
...
function CoachVisualWalkthrough({ lesson, topic }: { lesson: CoachLesson; topic: LearnTopic }) {
  ...
  const nodes = visualNodes.length >= 2 ? visualNodes : lesson.steps.map((step) => step.visualLabel).slice(0, 4);
  ...
  return <figure className={`coach-visual coach-visual-${kind}`} aria-label={`Visual walkthrough for ${topic.name}`}><figcaption><p className="text-[11px] font-bold uppercase tracking-[.16em] text-violet-700">Visual walkthrough</p><p className="mt-1 font-serif text-xl font-bold text-cocoa-900">{caption}</p></figcaption><div className="coach-visual-stage mt-4" aria-hidden><span className="coach-visual-spark coach-visual-spark-one">✦</span><span className="coach-visual-spark coach-visual-spark-two">✦</span><div className="coach-visual-path"><span /></div><div className="coach-visual-nodes">{nodes.map((node, index) => <div key={`${node}-${index}`} className="coach-visual-node"><span className="coach-visual-number">{index + 1}</span><span>{node}</span></div>)}</div></div><p className="mt-3 text-xs leading-5 text-cocoa-600">The sequence is an original study visual for this lesson—not a copied textbook diagram.</p></figure>;
}
