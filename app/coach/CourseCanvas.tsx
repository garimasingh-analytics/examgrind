  ...
  return <article className="course-canvas eg-page-enter mt-5 overflow-hidden rounded-[2rem] border border-cocoa-900/[.12] bg-[#fffaf0] shadow-warm-lg">
    ...
    <div className="course-canvas-body">
      ...
      <section className="course-canvas-teach" aria-live="polite">
        ...
        <aside className="mt-5 rounded-2xl border border-ember-700/15 bg-sun-50 px-4 py-3 text-sm leading-6 text-cocoa-800"><span className="font-mono text-[10px] font-extrabold uppercase tracking-[.12em] text-ember-700">See it in context</span><p className="mt-1">{step.example}</p></aside>
...
function HeartLessonPilot({ lesson, topic, visualAsset }: { lesson: Lesson; topic: Topic; visualAsset: VisualAsset }) {
  ...
  return <article className="heart-lesson eg-page-enter mt-5 overflow-hidden rounded-[2rem] border border-cocoa-900/[.12] bg-[#fffaf0] shadow-warm-lg">
    ...
    <section className="grid border-b border-cocoa-900/10 lg:grid-cols-[minmax(0,1.08fr)_minmax(19rem,.92fr)]">
      <figure className="bg-cream-50 p-5 sm:p-7">
        <figcaption className="mb-4"><p className="font-mono text-[10px] font-extrabold uppercase tracking-[.16em] text-ember-700">Read the actual figure</p><h4 className="mt-2 font-fraunces text-3xl font-bold tracking-[-.055em] text-cocoa-900">The heart is two pumps in one circuit.</h4></figcaption>
        ...
        <p className="mt-3 text-xs font-semibold leading-5 text-cocoa-700">Figure: <a className="text-ember-700 underline underline-offset-2" href={visualAsset.sourceUrl} target="_blank" rel="noreferrer">{visualAsset.sourceLabel}</a> · <a className="text-ember-700 underline underline-offset-2" href={visualAsset.licenceUrl} target="_blank" rel="noreferrer">{visualAsset.licenceLabel}</a> · {visualAsset.attribution}</p>
      </figure>
      <aside className="flex flex-col bg-[#fffdf8] p-5 sm:p-7">
        ...
        <div className="mt-4 grid gap-2" aria-label="Choose a point in the circulation route">{HEART_FLOW.map((item, index) => <button key={item.title} type="button" aria-pressed={focus === index} onClick={() => setFocus(index)} className={`rounded-xl border px-4 py-3 text-left transition ${focus === index ? "border-cocoa-900 bg-sun-200 text-cocoa-900 shadow-[3px_4px_0_rgba(29,24,21,.13)]" : "border-cocoa-900/10 bg-white text-cocoa-700 hover:border-ember-700/45"}`}><span className="block text-sm font-extrabold">{item.title}</span><span className="mt-1 block text-xs leading-5 opacity-80">{item.cue}</span></button>)}</div>
        <div className="mt-5 rounded-2xl border border-ember-700/15 bg-sun-50 p-4"><p className="font-mono text-[10px] font-extrabold uppercase tracking-[.12em] text-ember-700">What this means</p><p className="mt-2 text-sm leading-6 text-cocoa-800">{HEART_FLOW[focus].detail}</p></div>
      </aside>
    </section>
