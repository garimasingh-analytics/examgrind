...
export default async function CoachPage() {
  ...
  const strongest = [...signals].sort((a, b) => b.accuracy - a.accuracy)[0];
  const weekDirection = weakest
    ? `Repair ${weakest.topicName}, then revisit it after two days.`
    : coverageSubject
    ? `Build evidence in ${coverageSubject.name}, then use the result to set your next repair.`
    : "Complete one topic quiz so Coach can create your first evidence-based plan.";
  ...
  return (
    <main className="min-h-[100svh] bg-warm-wash pb-20">
      ...
      <section className="mx-auto mt-6 max-w-4xl px-5"><div className="rounded-3xl border border-sun-500/25 bg-sun-400/10 p-5"><p className="text-xs font-bold uppercase tracking-[.16em] text-ember-700">Your next 7 days</p><h2 className="mt-1 font-serif text-xl font-bold text-cocoa-900">The direction, not a rigid timetable.</h2><p className="mt-2 text-sm leading-6 text-cocoa-800">{weekDirection}</p><div className="mt-4 grid gap-2 sm:grid-cols-3"><p className="rounded-xl bg-cream-50 px-3 py-2 text-xs font-semibold text-cocoa-800">Days 1–2 · Repair the weakest concept</p><p className="rounded-xl bg-cream-50 px-3 py-2 text-xs font-semibold text-cocoa-800">Days 3–4 · Build coverage in the lagging subject</p><p className="rounded-xl bg-cream-50 px-3 py-2 text-xs font-semibold text-cocoa-800">Days 5–7 · Recall + take a mixed checkpoint</p></div>{strongest && <p className="mt-3 text-xs text-cocoa-600">Protect your strength too: {strongest.topicName} is currently your best-supported topic at {Math.round(strongest.accuracy * 100)}%.</p>}</div></section>
