...
export default function DiagnosisHandoffCard({ examSlug }: { examSlug: string }) {
    ...
    try {
      ...
      if (!hasTrackedView.current) {
        ...
        trackDiagnosisHandoffViewed({
          exam: parsed.exam as "cuet" | "ssc-cgl" | "neet-ug" | "delhi-police-constable" | "uppsc-ro-aro" | "up-secretariat-ro-aro" | "uiic-ao" | "uppsc-pcs",
  ...
  const dismiss = (action: "choose_subject" | "dismiss" = "dismiss") => {
    trackDiagnosisHandoffAction({
      exam: handoff.exam as "cuet" | "ssc-cgl" | "neet-ug" | "delhi-police-constable" | "uppsc-ro-aro" | "up-secretariat-ro-aro" | "uiic-ao" | "uppsc-pcs",
      action,
    });
    window.localStorage.removeItem(HANDOFF_KEY);
    setHandoff(null);
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 pt-4 sm:px-6">
      <div className="relative overflow-hidden rounded-3xl border border-coral-500/30 bg-cream-50 p-5 shadow-warm sm:p-6">
        ...
        <div className="flex items-start gap-4 pr-16">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-1 font-serif text-2xl font-semibold leading-tight tracking-[-.04em] text-cocoa-900 sm:text-3xl">Start with {handoff.concept}, or choose another subject.</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-cocoa-700">Your five answers pointed to {handoff.wrongCount === 1 ? "one topic to revisit" : `${handoff.wrongCount} topics to revisit`}. Use this as a starting point and study what you need today.</p>
            <div className="mt-4 flex flex-wrap gap-3">
              <Link href={`/subject/${handoff.subjectId}`} onClick={() => trackDiagnosisHandoffAction({ exam: handoff.exam as "cuet" | "ssc-cgl" | "neet-ug" | "delhi-police-constable" | "uppsc-ro-aro" | "up-secretariat-ro-aro" | "uiic-ao" | "uppsc-pcs", action: "follow_signal" })} className="eg-press inline-flex rounded-2xl bg-cocoa-900 px-4 py-2.5 text-sm font-bold text-cream-50 shadow-warm hover:bg-cocoa-700">Practise this topic →</Link>
              ...
              <Link href={`/guides?exam=${handoff.exam}`} onClick={() => { trackDiagnosisHandoffAction({ exam: handoff.exam as "cuet" | "ssc-cgl" | "neet-ug" | "delhi-police-constable" | "uppsc-ro-aro" | "up-secretariat-ro-aro" | "uiic-ao" | "uppsc-pcs", action: "read_guide" }); window.localStorage.removeItem(HANDOFF_KEY); }} className="inline-flex rounded-2xl border border-cocoa-900/15 bg-cream-50 px-4 py-2.5 text-sm font-bold text-cocoa-900 hover:bg-cream-100">Read my study guide</Link>
            </div>
          </div>
        </div>
      </div>
